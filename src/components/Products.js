...
class Products extends React.Component {
	...
	getDatas() {
		...
		const url = process.env.REACT_APP_URL + "api/book_stores?page=" + this.state.currentPage + "&order[id]=desc" + textSearch + "&book.book.language.abbreviation=es";
	...
	render() {console.log($this.state);
		...
		if (error) {
			...
			return <div className="text-center text-white"><div className="fa-3x"><i className="fas fa-spinner fa-pulse load-spinner"></i></div>Cargando…</div>;
									...
									<div className="card-body">
										...
										<div className="text-center">
										    <ProductDetail 
												...
												item={item}
											></ProductDetail>
											<a href={item.externalAmazonStoreLink} className="btn btn-sm amazon text-white"><i className="fab fa-amazon"></i> Cómpralo en Amazon</a>
										</div>
									</div>
								</div>
							</div>
